...
class PdfPage {
  ...
  function __construct($id, $title, $description, $image, $parent_page_id) {
    $this->id = $id;
    $this->title = $title; 
...
function get_all_pdf_pages() {
  ...
  $pdf_pages = array();
  $query = "SELECT * FROM pdf_pages ORDER BY title";
  ...
  while ($row = mysqli_fetch_assoc($pdf_page_info)) {
    $pdf_page = new PdfPage($row['id'], $row['title'], $row['description'], $row['page_image'], $row['parent_id']);
...
function get_pdf_pages_for_parent($parent_id) {
  ...
  $pdf_pages = array();
  $query = "SELECT * FROM pdf_pages WHERE parent_id=$parent_id ORDER BY title";
...
function add_pdf_page($title, $description, $image, $parent_page_id) {
  global $connection;

  $query = "INSERT INTO pdf_pages(title, description, parent_id, page_image)";
  $query .="VALUES('{$title}', '{$description}','{$parent_page_id}','{$image}')";
...
function update_pdf_page($pdf_page) {
  global $connection;
    ...
    $query .= " parent_id={$pdf_page->get_parent_page_id()}, page_image='{$pdf_page->get_image()}'";
...
class PdfFile {
  ...
  function __construct($id, $title, $thumbnail_image, $pdf_file, $pdf_page_id) {
    $this->title = $title; 
...
function get_pdf_files_for_page($pdf_page_id) {
  ...
  $query = "SELECT * FROM pdf_links WHERE pdf_page_id=$pdf_page_id";
  ...
  if ($count > 0) {
    while($row = mysqli_fetch_assoc($pdf_files)){
      $pdf_file = new PdfFile($row['id'], $row['title'], $row['pdf_image'], $row['pdf_filename'], $row['pdf_page_id']);
...
function add_pdf_file($title, $thumbnail_image, $pdf_filename, $pdf_page_id) {
  $query = "INSERT INTO pdf_links(title, pdf_filename, pdf_image, pdf_page_id)";
      
  $query .="VALUES('{$title}', '{$pdf_filename}','{$thumbnail_image}','{$pdf_page_id}')";

  error_log("Adding new PDF File: $pdf_filename, Thumbnail: $thumbnail_image", 0);
  $result = mysqli_query($connection, $query);
